...
# Gmail MCP 읽기 도구를 연결한 대화형 에이전트 (OAuth/MCP 설정 필요)
uv run python -m app.agent.cli
```

CLI는 실행 중 하나의 `thread_id`를 유지해 후속 질문의 대화 맥락을 보존한다. Step 4에서는
`list_messages`·`get_message` 읽기 도구만 연결되며, trash 도구는 승인(HITL) 기능이 추가되는
후속 단계에서 연결한다.
...
│  ├─ agent/             # LangGraph 에이전트 (create_agent, Step 4 읽기 도구)
│  ├─ mcp/               # Gmail MCP 연동
...
│  ├─ test_agent_cli.py     # Gmail 도구 연결 CLI 검증
